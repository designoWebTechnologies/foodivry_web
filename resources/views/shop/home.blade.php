            ...
            <div class="card-body">
                <div class="media">
                    ...
                    <div class="p-2 bg-gradient-x-warning white media-body">
                        ...
                        <h5 class="text-bold-400"><i class="ft-arrow-down"></i> {{currencydecimal($OrderIncomeToday)}}</h5>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="media">
                    ...
                    <div class="p-2 bg-gradient-x-success white media-body">
                        ...
                        <h5 class="text-bold-400"><i class="ft-arrow-up"></i> {{currencydecimal($OrderIncomeMonthly)}}</h5>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div id="recent-buyers" class="list-group height-300 position-relative">
                    ...
                    <a href="#" class="list-group-item list-group-item-action media no-border">
                        ...
                        <div class="media-body">
                            <h6 class="list-group-item-heading">{{$Order->user->name}} <span class="font-medium-4 float-xs-right pt-1">{{currencydecimal($Order->invoice->net)}}</span></h6>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if(!empty($Order->transporter->name))
                                    <td class="text-truncate">{{ucfirst(@$Order->transporter->name)}}</td>
                                @else
                                    <td class="text-truncate">-----</td>
                                @endif
                                <td class="text-truncate"><span class="tag tag-default tag-success">{{$Order->status}}</span></td>
                                <td class="text-truncate"> {{currencydecimal($Order->invoice->net)}}</td>
                            </tr>
                            @empty
    ...
    <script type="text/javascript">
        var data = [ 
            @foreach($complete_cancel as $k=>$v)
                    ...
                    {
                        month: "{{$v->monthdate}}",  delivered: {{$v->delivered}}, cancelled: {{$v->cancelled}}
                    },
