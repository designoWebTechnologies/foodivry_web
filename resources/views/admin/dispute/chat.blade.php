...
<div class="pro-dashboard-head">
    <div class="container">
        <a href="#" class="pro-head-link active"></a>
    </div>
</div>
...
<div class="panel panel-default" id="chat-place">
    <div class="panel-body">
        <h2 class="text-center">Chat with 
         @if(count($OrderDispute)>0)
            @if($OrderDispute->created_by == 'user')
                {{$Order->user->name}}
                <?php $user_id = $Order->user->id;
                 ?>
            @elseif($OrderDispute->created_by == 'transporter')
                {{$Order->user->name}}
               <?php  $user_id = $Order->user->id; ?>
            @elseif($OrderDispute->created_by == 'shop')
                {{$Order->user->name}}
               <?php  $user_id = $Order->user->id; ?>
            @endif
        @else
...
<script type="text/javascript">
    ...
    function initChat(){
        ...
        chatSockets.prototype.sendMessage = function(data) {
            ...
            data.provider_id = "{{ \Auth::guard('admin')->user()->id }}";
    ...
    function notifyuser(datamsg){
        //var data = JSON.stringify(datamsg)
        console.log(datamsg);
        $.post( "{{url('admin/pubnubnotify')}}",{'user_id':datamsg.user_id,'request_id':datamsg.request_id,'message':datamsg.message,'_token':"{{csrf_token()}}"}, function( data ) {
          //$( ".result" ).html( data );
