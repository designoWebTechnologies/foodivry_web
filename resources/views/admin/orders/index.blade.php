 ...
 <div class="card"> 
    <ul class="nav nav-tabs row m-0 common-tab">
        <div class="col-sm-3 p-0">
            <li class="nav-item">
                <a href="{{ route('admin.orders.index') }}?t=pending" class="nav-link @if(Request::get('t')=='pending') active  @endif">@lang('order.dispatcher.pending_title')</a>
            </li>
        </div>
        <div class="col-sm-3 p-0">
            <li class="nav-item">
                <a href="{{ route('admin.orders.index') }}?t=accepted" class="nav-link @if(Request::get('t')=='accepted') active  @endif">@lang('order.dispatcher.accept_title')</a>
            </li>
        </div>
        <div class="col-sm-3 p-0">
            <li class="nav-item">
                <a href="{{ route('admin.orders.index') }}?t=ongoing" class="nav-link @if(Request::get('t')=='ongoing') active  @endif">@lang('order.dispatcher.ongoing_title')</a>
            </li>
        </div>
        <div class="col-sm-3 p-0">
            <li class="nav-item">
                <a href="{{ route('admin.orders.index') }}?t=cancelled" class="nav-link @if(Request::get('t')=='cancelled') active  @endif">@lang('order.dispatcher.cancel_title')</a>
            </li>
        </div>
    </ul>
    <div class="card-header">
        @if(Request::get('t')=='pending')
            <h3 class="card-title   @if(Setting::get('demo_mode')==0) col-md-6  @endif ">@lang('order.dispatcher.order_pending')</h3>
        @endif
        @if(Request::get('t')=='accepted')
            <h3 class="card-title @if(Setting::get('demo_mode')==0) col-md-6  @endif ">@lang('order.dispatcher.order_accept')</h3>
        @endif
         @if(Request::get('t')=='ongoing')
            <h3 class="card-title @if(Setting::get('demo_mode')==0) col-md-6  @endif ">@lang('order.dispatcher.order_ongoing')</h3>
        @endif
         @if(Request::get('t')=='cancelled')
            <h3 class="card-title @if(Setting::get('demo_mode')==0) col-md-6  @endif ">@lang('order.dispatcher.order_cancel')</h3>
        @endif
        @if(Setting::get('demo_mode')==0)

        <span class="pull-right col-md-6">
            <div class="form-group col-xs-12">
            <form id="form_setting" role="form" method="POST" action="{{ route('admin.order-assign') }}" >
                
                {{ csrf_field() }}
                <label for="manual_assign" class="col-xs-4 "> @lang('setting.manual_assign') </label>
                <div class="col-xs-8">
                    <div class="float-xs-left mr-1"><input @if(Setting::get('manual_assign') == 0) checked  @endif  name="manual_assign" value="0" type="radio" class="js-switch" data-color="#43b968">Manual</div>
                    <div class="float-xs-left mr-1"><input @if(Setting::get('manual_assign') == 1) checked  @endif  name="manual_assign" value="1" type="radio" class="js-switch" data-color="#43b968">Automatic</div>
                </div>
            </form>
            </div>
        </span>
    ...
    @if(Request::get('t')=='pending')
    <div class="row">
        <form action="{{url('admin/orders?t=pending')}}" method="GET" class="popup-form" style="display:none;">
            <div class="col-md-6">
                <input type="hidden" name="t" class="form-control " value="pending" >
                <input type="text" name="delivery_date" class="form-control datepicker" placeholder="Date" readonly >
             </div>
             <div class="col-md-6">
    ...
    <div class="dispatcher row m-0">
        ...
        <div class="col-md-7">
                    ...
                    <div class="card card-inverse pending-block row m-0 @if($order_now == 1)  bg-primary @elseif($Order->status == 'RECEIVED') bg-danger  @else bg-success @endif">
                        ...
                        <div class="delete-block">
                            ...
                            <form action="{{ route('admin.orders.update', $Order->id) }}" id="assign-{{ $Order->id }}" class="form-horizontal" method="POST">
                                ...
                                <button class="btn @if($Order->status == 'CANCELLED') btn-success  @else btn-danger @endif btn-darken-3 del-icon" onclick="return confirm('Do You Want to Cancel this order?');">
            ...
            @elseif(Request::get('t')=='accepted')

                <!-- Pending Order Block Starts -->  
                @forelse($Orders->assigned() as $Order) 
                <?php 
                    $shop[$Order->shop->id]=$Order->shop;
                    $myorder[$Order->shop->id][]=$Order->id;  
                //dd($Orders);?>  
                    <div class="card card-inverse pending-block row m-0 bg-danger">
                        <div class="card-block">
                            ...
                            <div class="col-sm-9">
                                ...
                                <div class="card-btm row m-0">
                                    <button class="btn btn-danger btn-darken-3 orderlist" data-id="{{$Order->id}}" >Order List</button>
                                    @if($Order->status=='PROCESSING')
                                        <span class="tag batch">@lang('order.dispatcher.packed')</span>
                                    @else
                                        <span class="tag batch">@lang('order.dispatcher.assigned')</span>
                                    @endif
                                    <a href="{{ url('admin/chat?order_id='.$Order->id) }}" class="btn btn-danger btn-darken-3" target="_blank"  >Chat</a>
                                </div>
                            </div>
                ...
                @endforelse                  
                <!-- Pending Order Block Ends -->
            @elseif(Request::get('t')=='ongoing')

                <!-- Pending Order Block Starts -->  
                @forelse($Orders->ongoing() as $Order) 
                <?php 
                    $shop[$Order->shop->id]=$Order->shop;
                    $myorder[$Order->shop->id][]=$Order->id;  
                //dd($Orders);?>    
                        ...
                        <div class="card-block">
                            ...
                            <div class="col-sm-9">
                                ...
                                <div class="card-btm row m-0">
                                    <button class="btn btn-danger btn-darken-3 orderlist" data-id="{{$Order->id}}" >Order List</button>
                                     @if($Order->status=='PROCESSING')
                                        <form action="{{ route('admin.orders.update', $Order->id) }}" id="assign-{{ $Order->id }}" method="POST">
                                            {{ csrf_field() }}
                                            {{ method_field("PATCH") }}
            ...
            @else
                ...
                <!-- Pending Order Block Starts -->  
                @forelse($Orders->cancelled() as $Order) 
                <?php 
                    $shop[$Order->shop->id]=$Order->shop;
                    $myorder[$Order->shop->id][]=$Order->id;  
                //dd($Orders);?>   
                    <div class="card card-inverse pending-block row m-0 bg-danger">
                        <div class="card-block">
                            ...
                            <div class="col-sm-9">
                                ...
                                <div class="card-btm row m-0">
                                    <button class="btn btn-danger btn-darken-3 orderlist" data-id="{{$Order->id}}" >Order List</button>
                                    <a href="{{ url('admin/chat?order_id='.$Order->id) }}" target="_blank" class="btn btn-danger btn-darken-3">Chat</a>
                                </div>
                            </div>
                        </div>
                        <!-- <div class="delete-block">
        ...
        <audio id="beep-one" controls preload="auto" style="display:none">
            <source src="{{asset('public/assets/audio/beep.mp3')}}" controls></source>
            <source src="{{asset('public/assets/audio/beep.ogg')}}" controls></source>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <!-- <a href="{{url('downloadpdf/'.$Order->id)}}">Download PDF</a> -->
            ...
            <div class="modal-body">
                <div class="row m-0">
                    <dl class="order-modal-top">
                        ...
                        <div class="row m-0">
                            <dt class="col-sm-3 order-txt p-0">Order OTP</dt>
                            <dd class="col-sm-9 order-txt cust_order_otp"></dd>
                        </div>
...
@endsection
@section('styles')
<link rel="stylesheet" type="text/css" href="{{ asset('public/assets/css/timeTo.css')}}">
<link rel="stylesheet" type="text/css" href="{{ asset('public/assets/user/css/bootstrap-datetimepicker.min.css') }}">
@endsection

@section('scripts')
<script src="{{ asset('public/assets/js/jquery.time-to.js') }}" type="text/javascript"></script>
<script type="text/javascript" src="{{ asset('public/assets/user/js/bootstrap-datetimepicker.min.js') }}"></script>
...
$('.js-switch').on('change',function(){ 
        if(confirm('Do You Want to change Order Assign Method?')){
            $('#form_setting').submit();
        }else{
            location.reload();
        }
...
$(document).on("click", ".orderlist",function(){ 
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            if(result.Order.order_otp){
                $('.cust_order_otp').html('<span>: </span>'+result.Order.order_otp);
            }
            $('.address').html('<span>: </span>'+result.Order.address.map_address);
            $('.tot_amt').html('<span>: </span> {{Setting::get("currency")}}'+parseFloat(result.Order.invoice.net).toFixed(2));
            $('.discount').html("<span>: </span> {{Setting::get('currency')}}"+parseFloat(result.Order.invoice.discount).toFixed(2));
            $('.delivery_charge').html("<span>: </span> {{Setting::get('currency')}}"+parseFloat(result.Order.invoice.delivery_charge).toFixed(2));
            $('.tax').html("<span>: </span> {{Setting::get('currency')}}"+parseFloat(result.Order.invoice.tax).toFixed(2));
            var statuslist='';
            ...
            table +='<tr>';
                ...
                table +='<td>'+value.product.name+'</td><td>'+value.note+'</td><td>{{Setting::get('currency')}}'+parseFloat(value.product.prices.price).toFixed(2)+'</td><td>'+value.quantity+'</td><td>{{Setting::get('currency')}}'+(value.quantity*value.product.prices.price).toFixed(2)+'</td></tr>';
                    $.each(value.cart_addons, function( key, valuee ) { 
                            ...
                            <td>\
                                <strong>{{Setting::get('currency')}}"+parseFloat(valuee.addon_product.price).toFixed(2)+"</strong>\
                            </td>\
...
if (navigator.geolocation){
  ...
  console.log(showPosition); alert(111);
}
else{
  console.log("Geolocation is not supported by this browser.");
}
var cur_lat ='13.066239';
var cur_long ='80.274816';
function showPosition(position){ 
    location.latitude=position.coords.latitude;
    cur_lat = location.latitude;
    location.longitude=position.coords.longitude;
    cur_long = location.longitude; alert(cur_lat+'--'+cur_long);
    
} //showPosition
})*/
    function initialize() {

        var fenway = { lat: 13.066239, lng: 80.274816 };
     ...
     function ongoingInitialize(trip) {
        ...
        var marker = new google.maps.Marker({
            ...
            icon: '/public/assets/img/marker-start.png'
        });

        var markerSecond = new google.maps.Marker({
            map: map,
            anchorPoint: new google.maps.Point(0, -29),
            icon: '/public/assets/img/marker-end.png'
        });
        ...
        if(trip.transporter) {
            var markerProvider = new google.maps.Marker({
                map: map,
                icon: "/public/assets/img/marker-car.png",
                anchorPoint: new google.maps.Point(0, -29)
            });
...
<script src="https://maps.googleapis.com/maps/api/js?key={{Setting::get('GOOGLE_API_KEY')}}&callback=initialize" type="text/javascript"></script>
<script type="text/javascript">
    
    function assignorder(){ 
        $.ajax({
        url: "{{url('admin/orders?t=pending')}}&p=auto&ajax=1&auto=1",
        success: function(result){
           setTimeout(function(){ assignorder(); }, 3000);
            }
        });
    }
    assignorder();
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/react/0.13.3/react.js"></script>
    ...
    <script type="text/jsx">
         var total_order = 0 ;var curstatus = ''; var mode = '';
        ...
        var MainComponent = React.createClass({
            ...
            componentDidMount: function(){
                $.ajax({
                  url: "{{url('admin/orders?t='.Request::get('t'))}}&q=1",
                  type: "GET"})
                  .done(function(res){
            ...
            render: function(){
                ...
                var cancel_div =<div className="delete-block">
                            <form action={`{{url('admin/orders/')}}/${item.id}`} id="assign-item.id" onSubmit={mystatus} className="form-horizontal" method="POST">
                        ...
                        <div className="card-block">
                            ...
                            <div className="col-sm-9">
                                <div className="card-btm pending-btm">
                                    ...
                                    {mode==1?<p className="card-txt">{item.user.phone}</p>:<p className="card-txt">{item.user.phone}</p>
                                    }
                                </div>
