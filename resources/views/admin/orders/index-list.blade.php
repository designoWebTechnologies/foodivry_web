 ...
 <div class="card">
    ...
    <div class="card-body collapse in">
                ...
                <div class="form-group col-xs-6 ">
                    ...
                    <select id="user_name" name="sp" class="form-control">
                        ...
                        @forelse($Users as $id=>$name)
                            <option value="{{$id}}"  @if(Request::get('sp')==$id) selected  @endif  >{{$name}}</option>
                        @empty
                        @endforelse
                    </select>
                </div>
            ...
            <div class="form-group col-xs-12 mb-2">
                <div class="form-group col-xs-6 ">
                    ...
                    <select id="user_name" name="st" class="form-control">
                        <option value="">Select</option>
                        <option value="completed"  @if(Request::get('st')=='completed') selected  @endif  >Completed</option>        
                        <option value="cancelled"  @if(Request::get('st')=='cancelled') selected  @endif  >Cancelled</option>
                    </select>
                </div>
                <div class="form-group col-xs-6 ">
                    ...
                    <select id="user_name" name="pm" class="form-control">
                         <option value="">Select</option>
                        <option value="cash" @if(Request::get('pm')=='cash') selected  @endif  >Cash</option>
                    </select>
                </div>
            </div>
        ...
        <div class="card-block card-dashboard table-responsive">
            <div><a href="{{(Request::getPathInfo() .'?'. (Request::getQueryString().'&exl=1'))}}"  class="btn btn-primary">Export As Excel</a></div>
            <br/>
            <table class="table table-striped table-bordered file-export">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if(!empty($Order->transporter->name))
                        ...
                        <td><span class="tag @if($Order->status=='COMPLETED') tag-success 
                        @elseif($Order->status=='CANCELLED') tag-danger @else tag-primary  @endif
            ...
            <div class="modal-body">
                <div class="row m-0">
                    <dl class="order-modal-top">
                        ...
                        <hr/>
                        <div style="font-size: large;">User</div>
                        <div class="row m-0">
                            <dt class="col-sm-3 order-txt p-0 ">Rating</dt>
                            <dd class="col-sm-9 order-txt rate_user"><span>: </span> No Rating</dd>
                        </div>
                        <div class="row m-0">
                            <dt class="col-sm-3 order-txt p-0">Review</dt>
                            <dd class="col-sm-9 order-txt comment_user"><span>: </span> No Review</dd>
                        </div>
                        <hr/>
                        <div style="font-size: large;">Provider</div>
                        <div class="row m-0">
                            <dt class="col-sm-3 order-txt p-0">Rating</dt>
                            <dd class="col-sm-9 order-txt rate_deliveryboy"><span>: </span> No Rating</dd>
                        </div>
                        <div class="row m-0">
                            <dt class="col-sm-3 order-txt p-0">Review</dt>
                            <dd class="col-sm-9 order-txt comment_transporter"><span>: </span> No Review</dd>
                        </div>
                        <hr/>
                         <div style="font-size: large;">Shop</div>
                        <div class="row m-0">
                            <dt class="col-sm-3 order-txt p-0">Rating</dt>
                            <dd class="col-sm-9 order-txt rate_shop"><span>: </span> No Rating</dd>
                        </div>
                        <div class="row m-0">
                            <dt class="col-sm-3 order-txt p-0">Review</dt>
                            <dd class="col-sm-9 order-txt comment_shop"><span>: </span> No Review</dd>
                        </div>
                        <hr/>
                        <div class="row m-0" >
...
@endsection
@section('styles')
<link rel="stylesheet" type="text/css" href="{{ asset('public/assets/admin/plugins/bootstrap-datepicker/css/bootstrap-datepicker.min.css')}}">
@endsection
@section('scripts')
            ...
            @endif
             if(result.Order.reviewrating){
                if(result.Order.reviewrating.shop_rating){
                    var rate = '<span class="rating">';
                        for(var i=1;i<=result.Order.reviewrating.shop_rating;i++){
                        rate+='<i class="fa fa-star" aria-hidden="true" style="color:#ffa500"></i>';
                        }
            ...
            if(result.Order.reviewrating){
             ...
             if(result.Order.reviewrating){
                if(result.Order.reviewrating.user_rating){
                    var rate = '<span class="rating">';
                        for(var i=1;i<=result.Order.reviewrating.user_rating;i++){
                        rate+='<i class="fa fa-star" aria-hidden="true" style="color:#ffa500"></i>';
                        }
                        for(var i=1;i<=(5-result.Order.reviewrating.user_rating);i++){
                        rate+='<i class="fa fa-star-o" aria-hidden="true"></i>';
                        }
                        rate+='</span>';
                    $('.rate_user').html('<span>: </span>'+rate);
                }else{
                    $('.rate_user').html('<span>: -- </span>');
                }
            }else{
               $('.rate_user').html('<span>: -- </span>'); 
            }
            if(result.Order.reviewrating){
                $('.comment_shop').html('<span>: </span>'+result.Order.reviewrating.shop_comment);
            }else{
                $('.comment_shop').html('<span>: -- </span>');
            }
            if(result.Order.reviewrating){
                $('.comment_user').html('<span>: </span>'+result.Order.reviewrating.shop_comment);
            }else{
                $('.comment_user').html('<span>: -- </span>');
            }
            if(result.Order.reviewrating){
                $('.comment_transporter').html('<span>: </span>'+result.Order.reviewrating.transporter_comment);
            }else{
                $('.comment_transporter').html('<span>: -- </span>');
            }
