...
<div class="card">
    <div class="card-header">
        ...
        @else
        <h3 class="card-title">Edit Dispute Manager</h3>
        @endif
    </div>
    <div class="card-body collapse in">
        <div class="card-block">
            <form class="form-horizontal" role="form" method="POST" action="{{ route('admin.dispute-user.update', $User->id) }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                ...
                <div class="form-group col-xs-12 mb-2">
                    <input id="password" type="password" class="form-control" name="password" @if(Setting::get('DEMO_MODE')==0) {{'readonly'}} @endif placeholder="Password">
                    @if(Setting::get('DEMO_MODE')==0)
                        <span class="help-block">
                            <strong>{{ "Change Password not available for demo mode" }}</strong>
                        </span>
                    @endif
                ...
                <div class="form-group col-xs-12 mb-2">
                     <input id="password-confirm" type="password" class="form-control" @if(Setting::get('DEMO_MODE')==0) {{'readonly'}} @endif name="password_confirmation" placeholder="Confirm Password" >
                </div>
                <div class="form-group col-xs-12 mb-2">
                ...
                <div class="col-xs-12 mb-2">
                    <a href="{{ route('admin.dispute-user.index') }}" class="btn btn-warning mr-1">
