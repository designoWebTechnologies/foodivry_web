                    ...
                    @forelse($TransporterShifts as $Index => $Shift)
                           ...
                           <?php //$total=0; ?>
                            ...
                            @if($ShiftOrder->shift_id)
                                ...
                                <tr>
                                    ...
                                    <td>{{ @$ShiftOrder->address->map_address }}</td>
                        ...
                        @forelse($Shift as $ShiftIndex => $ShiftOrder)
                             @if($ShiftOrder->start_time && !$ShiftOrder->transporter_shift_id)
                                <tr><td colspan="4">Shift Start Time:{{$ShiftOrder->start_time}}</td><td colspan="4">Shift End Time:{{$ShiftOrder->end_time ? $ShiftOrder->end_time : ' -- '}}</td></tr><tr><td colspan="4"></td><td colspan="4"> Total amount :{{@$ShiftOrder->total}}</td></tr>
                            @endif
                            @if($ShiftOrder->transporter_shift_id)
                                <tr><td colspan="4">Break Start Time:{{$ShiftOrder->start_time}}</td><td colspan="4">Break End Time:{{$ShiftOrder->end_time ? $ShiftOrder->end_time : ' --'}}</td></tr>
                            @endif
        ...
        success: function(result){
           ...
           // alert(isEmpty(reviewrating));
            ...
            $.each( result.Cart, function( key, value ) {
                ...
                table +='<td>'+value.product.name+'</td><td>{{Setting::get('currency')}}'+parseFloat(value.product.prices.price).toFixed(2)+'</td><td>'+value.quantity+'</td><td>{{Setting::get('currency')}}'+(value.quantity*value.product.prices.price).toFixed(2)+'</td></tr>';
